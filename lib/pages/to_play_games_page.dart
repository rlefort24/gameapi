...
class ToPlayGamesPage extends StatefulWidget {
  const ToPlayGamesPage({super.key});

  @override
  State<ToPlayGamesPage> createState() => _ToPlayGamesPageState();
}

class _ToPlayGamesPageState extends State<ToPlayGamesPage> {
  ...
  Future<List<Game>> _loadGames() async {
    try {
      final gameIds = await _supabaseService.getToPlayGames();
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(
              child: CircularProgressIndicator(
                color: Color.fromARGB(255, 1, 99, 190),
              ),
            );
